...
def load_solver(args, operator, device):
    if args.solver=="unrolled":
        ...
        return SIUPPA(linear_operator=operator, nb_channels=int(args.nb_channels), pretrained=args.pretrained, nb_samples_per_inference=int(args.nb_samples_per_inference), solver=args.fp_algorithm, max_iter=int(args.max_iter), tol=float(args.tol), penalty_initial_val=float(args.penalty), sigma_initial_val=float(args.sigma), device=device).to(device=device)
    else:
