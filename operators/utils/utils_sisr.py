...
def pre_calculate_cmplx(y, FB, sf, device):
    '''
    ...
    FBFy = FBC * torch.fft.fftn(y, dim=(-2, -1)).repeat(1, 1, sf, sf).to(device)
    return FBC, F2B, FBFy
...
def data_solution_cmplx(x, FB, FBC, F2B, FBFy, rho, sf, device):
    Fx = torch.fft.fftn(x, dim=(-2, -1)).to(device)
    ...
    #Fx += (FBFy - FBC * ax) / (aaT + rho)
    Xproj = torch.fft.irfftn(Fx, Fx.shape[-2:], dim=(-2, -1))
    return Xproj

def project_operator_cmplx(x, FB, FBC, F2B, FBFy, sf):
